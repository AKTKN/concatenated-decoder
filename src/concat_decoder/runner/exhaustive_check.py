...
def run_exhaustive_check(
    ...
    if weight >= 3:
        warnings.warn(
            f"[concat-decoder][exhaustive_check] weight={weight} will be very expensive: patterns=C({num_mech},{weight})={num_patterns}",
            RuntimeWarning,
            stacklevel=2,
        )
    elif num_patterns >= 5_000_000:
        warnings.warn(
            f"[concat-decoder][exhaustive_check] patterns=C({num_mech},{weight})={num_patterns} is very large; this may take a long time.",
            RuntimeWarning,
            stacklevel=2,
        )
    ...
    if num_obs == 0:
        warnings.warn(
            "[concat-decoder][exhaustive_check] Circuit has num_observables=0; logical success/failure cannot be assessed.",
            RuntimeWarning,
            stacklevel=2,
        )
