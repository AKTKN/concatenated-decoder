...
def parse_detector_metadata(circuit: stim.Circuit) -> ParsedDetectorMetadata:
    ...
    for det_id, c in coords.items():
        if len(c) >= 5:
            ...
            # Chromobius format: 4th coord encodes color+basis.
            # Convention: 0=rX,1=gX,2=bX,3=rZ,4=gZ,5=bZ, -1=ignore.
            k = int(round(c[3]))

            if k not in chromobius_color_map:
                # k == -1 (ignore) or any unknown value → skip detector
                continue
            ...
            # Fewer than 4 coordinates → no color/basis annotation; skip and
            # collect for a single consolidated warning below.
            short_coord_ids.append(det_id)
            continue
    ...
    if short_coord_ids:
        warnings.warn(
            f"[concat-decoder] {len(short_coord_ids)} detector(s) have fewer than 4 coordinates "
            f"and carry no color/basis annotation — they will be ignored by the decoder. "
            f"This can happen when the circuit was generated by a library other than Chromobius "
            f"or color-code-stim (e.g. a generic stim circuit). "
            f"Affected detector IDs (first 10): {sorted(short_coord_ids)[:10]}"
            + (" ..." if len(short_coord_ids) > 10 else ""),
            UserWarning,
            stacklevel=2,
        )
