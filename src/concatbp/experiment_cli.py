...
# Import configs from the newly separated config module
...
from .visualization import threshold_plot_paths
...
def _load_config(path: Path) -> tuple[ThresholdExperimentConfig, DecoderConfig]:
    ...
    # Backward compatibility: migrate legacy field circuit_kind -> circuit_style.
...
def main() -> None:
    ...
    # Apply CLI overrides to the frozen dataclass
    if (
    ...
    # Output paths matching results_io.py and visualization.py
    out_dir = Path(exp_cfg.output_dir)
    plot_paths = threshold_plot_paths(out_dir, exp_cfg.plot_ler_unit, exp_cfg.plot_x_scale)

    print(f"\n[concatbp] Finished memory experiment: {len(rows)} points evaluated.")
    print(f"[concatbp] Results CSV: {out_dir / 'threshold_results.csv'}")
    
    if dec_cfg.hybrid_two_stage.detailed_stats:
        print(f"[concatbp] Detailed Stats: {out_dir / 'detailed_stats.parquet'}")
    ...
    if bool(getattr(exp_cfg.detailed_stats, "enabled", False)):
        ...
        print(f"[concatbp] Shot-level Detailed Stats Dir: {out_dir / subdir}")
        
    print("[concatbp] Generated Plots:")
    for p in plot_paths:
        print(f"  - {p}")
